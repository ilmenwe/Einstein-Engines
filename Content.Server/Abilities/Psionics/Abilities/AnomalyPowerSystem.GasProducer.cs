...
public sealed partial class AnomalyPowerSystem
{
    ...
    private void GasProducerSupercrit(EntityUid uid, PsionicComponent component, AnomalyPowerActionEvent args)
    {
        ...
        var tilerefs = _mapSystem.GetLocalTilesIntersecting(xform!.GridUid.Value, grid, new Box2(localpos + new Vector2(-radius, -radius), localpos + new Vector2(radius, radius))).ToArray();
    ...
    private void GasProducerPulse(EntityUid uid, PsionicComponent component, AnomalyPowerActionEvent args)
    {
        ...
        var localpos = xform.Coordinates.Position;
        var tilerefs = _mapSystem.GetLocalTilesIntersecting(uid, grid, new Box2(localpos + new Vector2(-radius, -radius), localpos + new Vector2(radius, radius))).ToArray();
