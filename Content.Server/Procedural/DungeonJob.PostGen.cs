...
public sealed partial class DungeonJob
{
    ...
    private async Task PostGen(BoundaryWallPostGen gen, Dungeon dungeon, EntityUid gridUid, MapGridComponent grid, Random random)
    {
        ...
        foreach (var neighbor in dungeon.RoomExteriorTiles)
        {
            ...
            if (!_anchorable.TileFree(gridUid,grid, neighbor, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                continue;
        ...
        foreach (var index in dungeon.CorridorExteriorTiles)
        {
            ...
            if (!_anchorable.TileFree(gridUid, grid, index, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                continue;
        ...
        {
            var index = tiles[i];
            if (!_anchorable.TileFree(gridUid, grid, index.Index, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                continue;
        ...
        for (var i = 0; i < gen.Count; i++)
        {
            ...
            foreach (var tile in roomTiles)
            {
                ...
                for (var j = 0; j < 4; j++)
                {
                    ...
                    if (!_anchorable.TileFree(gridUid, _grid, tile, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        continue;
                    }

                    // Check if interior spot free (no guarantees on exterior but ClearDoor should handle it)
                    if (!_anchorable.TileFree(gridUid, _grid, dirVec, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        continue;
                    }
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                for (var j = -1; j < 4; j++)
                {
                    ...
                    if (!allExterior.Contains(neighbor) ||
                        ...
                        !_anchorable.TileFree(gridUid, grid, neighbor, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        isValid = false;
                        break;
                    }
                    ...
                    foreach (var k in new [] {2, 6})
                    {
                        ...
                        if (allExterior.Contains(perpTile) ||
                            takenTiles.Contains(neighbor) ||
                            !_anchorable.TileFree(gridUid, _grid, perpTile, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                        {
                            isValid = false;
                            break;
                        }
        ...
        foreach (var room in dungeon.Rooms)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                foreach (var tile in room.Tiles)
                {
                    ...
                    if (!_anchorable.TileFree(gridUid, grid, windowTile, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                        continue;
        ...
        foreach (var tile in dungeon.CorridorTiles)
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                for (var j = -width; j <= width; j++)
                {
                    ...
                    if (!_anchorable.TileFree(gridUid, _grid, neighbor, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        isValid = false;
                        break;
                    }
                    ...
                    if (!_anchorable.TileFree(gridUid, _grid, perp1, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        isValid = false;
                        break;
                    }

                    if (!_anchorable.TileFree(gridUid, _grid, perp2, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                    {
                        isValid = false;
                        break;
                    }
                ...
                foreach (var j in new [] {-exteriorWidth, exteriorWidth})
                {
                    ...
                    for (var k = 0; k < 4; k++)
                    {
                        ...
                        if (_anchorable.TileFree(gridUid, _grid, cornerNeighbor, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                        {
                            freeCount++;
                        }
                    }
        ...
        foreach (var (room, border) in roomBorders)
        {
            ...
            foreach (var (otherRoom, otherBorders) in roomBorders)
            {
                ...
                for (var i = 0; i < nodeDistances.Count; i++)
                {
                    ...
                    if (!_anchorable.TileFree(gridUid, grid, node, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                        continue;
        ...
        Random random)
    {
        ...
        {
            // Occupado
            if (dungeon.RoomTiles.Contains(neighbor) || checkedTiles.Contains(neighbor) || !_anchorable.TileFree(gridUid, grid, neighbor, DungeonSystem.CollisionLayer, DungeonSystem.CollisionMask))
                continue;
